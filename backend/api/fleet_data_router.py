...
def get_devices(
    ...
    """Get all devices with optional filtering (Manager only)."""
...
def create_device(
    ...
    """Create new device (Manager only)."""
...
def update_device(
    ...
    if device_update.device_number and device_update.device_number != device.device_number:
        existing = db.query(Device).filter(Device.device_number == device_update.device_number).first()
        if existing:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
def get_dashboard_stats(
    ...
    """Get dashboard statistics for Fleet Data Manager (Manager only)."""
