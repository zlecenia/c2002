...
def get_system_configs(
    ...
    for config in configs:
        ...
        if len(key_parts) >= 3:
            ...
            config_name_extracted = (
                ".".join(key_parts[1:]) if len(key_parts) > 1 else config.config_key
            )
            is_active = True
...
def create_system_config(
    ...
    existing = (
        ...
        .filter(Configuration.config_key == config_key, Configuration.component == "FCM")
        .first()
    )
    if existing:
        raise HTTPException(
    ...
    return {
        ...
        "created_at": db_config.config_value.get("metadata", {}).get(
            "created_at", db_config.updated_at
        ),
...
def get_system_config(
    ...
    config = (
        db.query(Configuration)
        .filter(Configuration.id == config_id, Configuration.component == "FCM")
        .first()
    )

    if not config:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="System configuration not found"
        )
...
def update_system_config(
    ...
    if isinstance(current_config_value, dict) and "metadata" in current_config_value:
        ...
        current_metadata = {
            "is_active": True,
            "description": "",
            "created_at": config.updated_at.isoformat(),
        }

    # Update configuration data and metadata
    new_data = (
        config_update.config_value if config_update.config_value is not None else current_data
    )
    ...
    enhanced_config_value = {"data": new_data, "metadata": new_metadata}
    ...
    if len(current_key_parts) >= 3:
        ...
        current_name = (
            ".".join(current_key_parts[1:]) if len(current_key_parts) > 1 else config.config_key
        )
    ...
    if new_key != config.config_key:
        existing = (
            db.query(Configuration)
            .filter(Configuration.config_key == new_key, Configuration.component == "FCM")
            .first()
        )
        if existing:
            raise HTTPException(
...
def get_device_configs(
    current_user: User = Depends(require_role("configurator")), db: Session = Depends(get_db)
):
...
def create_test_scenario_config(
    ...
    existing = (
        db.query(TestScenario).filter(TestScenario.name == scenario_config.scenario_name).first()
    )
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
def update_test_scenario_config(
    ...
    if scenario_update.scenario_name and scenario_update.scenario_name != scenario.scenario_name:
        existing = (
            ...
            .filter(TestScenario.scenario_name == scenario_update.scenario_name)
            .first()
        )
        if existing:
            raise HTTPException(
...
def get_config_dashboard(
    ...
    return {
        ...
        "configuration_coverage": round(
            (configured_devices / total_devices * 100) if total_devices > 0 else 0, 2
        ),
    }
...
def backup_configurations(
    ...
    devices = db.query(Device).all()
    device_configs = [
        {"id": d.id, "configuration": d.configuration if d.configuration is not None else {}}
        for d in devices
    ]
    ...
    scenarios = db.query(TestScenario).all()
    scenario_configs = [
        {"id": s.id, "parameters": s.parameters or {}, "expected_results": s.expected_results or {}}
        for s in scenarios
    ]
    ...
    try:
        ...
        if "test_scenario_configurations" in backup_data:
            for scenario_config in backup_data["test_scenario_configurations"]:
                scenario = (
                    db.query(TestScenario).filter(TestScenario.id == scenario_config["id"]).first()
                )
                if scenario:
        ...
        if "system_configurations" in backup_data:
            for system_config in backup_data["system_configurations"]:
                config = (
                    db.query(Configuration)
                    .filter(
                        Configuration.id == system_config["id"], Configuration.component == "FCM"
                    )
                    .first()
                )
                if config:
...
def get_json_template(
    ...
    if not template:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="JSON template not found")
    return template
...
def delete_json_template(
    ...
    return {"message": "JSON template deleted successfully", "template_id": template_id}
