...
def test_get_dashboard_stats(api_url, manager_token):
    ...
    response = requests.get(
        f"{api_url}/fleet-data/dashboard",
        headers=headers
    )
...
def test_update_device(api_url, manager_token):
    ...
    response = requests.put(
        f"{api_url}/fleet-data/devices/{device_id}",
        headers=headers,
        json=update_data
    )
    assert response.status_code == 200
...
def test_list_customers(api_url, manager_token):
    ...
    headers = {"Authorization": f"Bearer {manager_token}"}
    response = requests.get(
        f"{api_url}/fleet-data/customers",
        headers=headers
    )
    assert response.status_code == 200
...
def test_device_filtering(api_url, manager_token):
    ...
    headers = {"Authorization": f"Bearer {manager_token}"}
    response = requests.get(
        f"{api_url}/fleet-data/devices?device_type=mask_tester",
        headers=headers
    )
    assert response.status_code == 200
    devices = response.json()
