...
def test_list_system_configs(api_url, configurator_token):
    ...
    response = requests.get(
        f"{api_url}/fleet-config/system-configs",
        headers=headers
    )
...
def test_update_system_config(api_url, configurator_token):
    ...
    # Update config
    update_data = {
        "config_value": {"updated": "new_value"},
        "description": "Updated"
    }
    response = requests.put(
        f"{api_url}/fleet-config/system-configs/{config_id}",
        headers=headers,
        json=update_data
    )
    assert response.status_code == 200
...
def test_list_json_templates(api_url, configurator_token):
    ...
    headers = {"Authorization": f"Bearer {configurator_token}"}
    response = requests.get(
        f"{api_url}/fleet-config/json-templates",
        headers=headers
    )
    assert response.status_code == 200
...
def test_filter_json_templates_by_type(api_url, configurator_token):
    ...
    headers = {"Authorization": f"Bearer {configurator_token}"}
    response = requests.get(
        f"{api_url}/fleet-config/json-templates?type=test_flow",
        headers=headers
    )
    assert response.status_code == 200
    templates = response.json()
