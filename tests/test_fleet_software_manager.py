...
def test_get_software_dashboard_stats(api_url, auth_headers):
    ...
    response = requests.get(f"{api_url}/fleet-software/dashboard/stats", headers=auth_headers)
...
def test_list_software(api_url, auth_headers):
    ...
    response = requests.get(f"{api_url}/fleet-software/software", headers=auth_headers)
    assert response.status_code == 200
...
def test_create_software(api_url, auth_headers):
    ...
    response = requests.post(
        f"{api_url}/fleet-software/software", headers=auth_headers, json=software_data
    )
    assert response.status_code == 200
    data = response.json()
...
def test_update_software(api_url, auth_headers):
    ...
    # Update the software
    update_data = {"description": "Updated description", "current_version": "1.1.0"}
    response = requests.put(
        f"{api_url}/fleet-software/software/{software_id}", headers=auth_headers, json=update_data
    )
    assert response.status_code == 200
    data = response.json()
...
def test_delete_software(api_url, auth_headers):
    ...
    response = requests.delete(
        f"{api_url}/fleet-software/software/{software_id}", headers=auth_headers
    )
    assert response.status_code == 200
...
def test_get_software_versions(api_url, auth_headers):
    ...
    response = requests.get(
        f"{api_url}/fleet-software/software/{software_id}/versions", headers=auth_headers
    )
    assert response.status_code == 200
...
def test_create_software_version(api_url, auth_headers):
    ...
    # Create a version
    version_data = {"version_number": "2.0.0", "release_notes": "Major update", "is_stable": True}
    response = requests.post(
...
def test_list_installations(api_url, auth_headers):
    ...
    response = requests.get(f"{api_url}/fleet-software/installations", headers=auth_headers)
    assert response.status_code == 200
